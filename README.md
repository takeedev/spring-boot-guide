...
## 12. เจาะลึก @Transactional — all-or-nothing กับฐานข้อมูล

### 12.1 Transaction คืออะไร?
...
### 12.2 @Transactional ทำอะไรให้เรา?
...
### 12.3 ทำงานยังไงเบื้องหลัง? — ความลับคือ "Proxy"

เชื่อมกับเรื่อง Bean ในข้อ 11: Spring **ไม่ได้**ฉีด `TransferService` ตัวจริงให้ Controller
...
### 12.4 กับดักที่เจอบ่อยมาก (สำคัญ!)
...
## 13. จากโค้ดฝึกหัดสู่งานจริง — 5 เรื่องที่โค้ดบริษัทต่างจากโค้ดฝึก

### 13.1 DTO — อย่าส่ง Entity ออกไปตรง ๆ
...
### 13.2 ต่อ database จริง (PostgreSQL) + Flyway
...
### 13.3 Profiles — แยก config ตามสภาพแวดล้อม
...
### 13.4 Logging — เลิกใช้ System.out.println
...
### 13.5 Lombok — ลดโค้ด boilerplate
...
## 14. สิ่งที่เจอบ่อยในงานจริง

### 14.1 Pagination — แบ่งหน้าข้อมูล
...
### 14.2 JWT Authentication — login แบบ token

Basic Auth (ข้อ 10) ส่ง username/password ทุก request — ใช้ฝึกได้แต่งานจริงใช้ **JWT**:
...
ส่วนประกอบที่ต้องเขียนเพิ่มจากข้อ 10: `AuthController` (รับ login แจก token), `JwtService` (สร้าง/ตรวจ token — ใช้ library `jjwt`), และ `JwtAuthenticationFilter` (ดักทุก request ตรวจ header) — แนะนำหาบทความ "Spring Security JWT" ประกอบตอนลงมือทำ

### 14.3 Swagger / OpenAPI — เอกสาร API อัตโนมัติ
...
### 14.4 เรียก API ภายนอก — RestClient และ @HttpExchange
...
### 14.5 Spring Boot Actuator — ตรวจสุขภาพแอป
...
## 15. Annotation ชุดถัดไป — ตัวเดียวจบต่อเรื่อง

กลุ่มนี้เรียนง่ายสุด: แต่ละตัวเปิดความสามารถใหม่ให้ method ธรรมดา ๆ ทันที

### 15.1 @Scheduled — สั่งรันงานตามเวลา
...
### 15.2 @Async — ทำงานเบื้องหลัง ไม่ให้ user รอ
...
> ⚠️ `@Async` ทำงานผ่าน **proxy เหมือน @Transactional** (ข้อ 12.3) — เรียกจากใน class เดียวกัน (`this.xxx()`) จะไม่ async!

### 15.3 @Cacheable / @CacheEvict — จำผลลัพธ์ ไม่ query ซ้ำ
...
### 15.4 @EventListener — แยก logic ออกจากกันด้วย event
...
> 📌 สังเกต: `@Scheduled`, `@Async`, `@Cacheable` ทั้งหมดทำงานผ่าน **proxy** แบบเดียวกับ `@Transactional` — กับดัก self-invocation (ข้อ 12.4) ใช้กับพวกนี้ด้วยทุกตัว
